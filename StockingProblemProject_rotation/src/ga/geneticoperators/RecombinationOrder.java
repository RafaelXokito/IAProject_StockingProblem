...
public class RecombinationOrder<I extends IntVectorIndividual, P extends Problem<I>> extends Recombination<I, P> {
    ...
    public void crossOver(int[] offspring, I ind) {
        ...
        int index = 0;
        for (int i = 1; i < offspring.length-cut2-1;i+=2) {
            do {
                offspring[i + cut2] = sequence[index % sequence.length];
                offspring[i + cut2 + 1] = sequence[(index+1) % sequence.length];
                index+=2;
            }while (check_forDuplicates(offspring, i + cut2));
        }

        //acrescentar a parte direita (antes do cut1) vindo da sequencia
        for (int i = 0; i < cut1;i+=2) {
            do {
                offspring[i] = sequence[index % sequence.length];
                offspring[i+1] = sequence[(index+1) % sequence.length];
                index+=2;
