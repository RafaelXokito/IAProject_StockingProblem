...
public class MutationDisplacement<I extends IntVectorIndividual, P extends Problem<I>> extends Mutation<I, P> {
    ...
    public void mutate(I ind) {
        ...
        int position1;
        do {
            position1 = GeneticAlgorithm.random.nextInt(ind.getNumGenes());
        }while(position1 % 2 != 0);
        ...
        //Preenchimento do subSequenceAux valores que estão depois do cut2 até antes do cut 1
        ...
        if (subSequenceAux.length > 0)
        for (int i = (subSequenceSize + position1 > mutatedSequence.length) ? (subSequenceSize + position1) % mutatedSequence.length : 0, j = 0; (i < mutatedSequence.length && j < subSequenceAux.length); i+=2) {
            if(i < position1 || i > position2){
                mutatedSequence[i] = subSequenceAux[j++];
                mutatedSequence[i+1] = subSequenceAux[j++];
            }
        }
